...
BAD_PATTERNS = [
    rf"\b{PREFIX}ху(й|їла|йня|йло|йняк|йовий|єглот|єглотка|єсос|єсоска|йлан|йланка|єсосіна){SUFFIX}\b",
    ...
    rf"\b{PREFIX}[єеї]б(а|ать|нути|ан|лан|нутись|ат|анутись|анути|ланоїд|ланка|лун){SUFFIX}\b",
    rf"\b{PREFIX}бля(дь|ха|діна|дина|ть|т|буду|ді){SUFFIX}\b",
    rf"\b{PREFIX}су(ка|чка|чара)?{SUFFIX}\b",
    rf"\b{PREFIX}на(хуй|хуя|хуярився|хуйнувся|єбнувся|єбнулась|дрочився|дрочив){SUFFIX}\b",
    rf"\b{PREFIX}під(ар|ор|арас|орас|ріла|р|рілка|арасіна|арасина){SUFFIX}\b",
    rf"\b{PREFIX}пид(ар|ор|арас|орас|ріла|р|рілка|арасина|арасіна){SUFFIX}\b",
    rf"\b{PREFIX}уйобок{SUFFIX}\b",
    rf"\b{PREFIX}ган(дон|дурас|дончик|дурасик){SUFFIX}\b",
    rf"\b{PREFIX}шл(юшка|юха|ендра|ьондра){SUFFIX}\b",
...
async def log_all_messages(msg: Message):
    ...
    if text and BAD_REGEX.search(text):
        log_swear(chat_id, user_id, username)
